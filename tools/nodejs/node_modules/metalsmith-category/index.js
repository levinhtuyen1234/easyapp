...
function paginate(files, metalsmith, done, categoryOption) {
    var metadata = metalsmith.metadata();
    // Iterate over all the paginate names and match with collections.
    var complete = Object.keys(metadata.AllCategory).every(function (name) {
        var collection;
        ...
        //} catch (error) { }
        collection = metadata.AllCategory[name];
        ...
        if (categoryOption[name])
            ...
            pageOptions = extend(DEFAULTS, categoryOption['default']);
...
module.exports = function (opts) {
    return function (files, metalsmith, done) {
        ...
        for (var filePath in files) {
            ...
            //category.default.files.push(data); // add to default
            ...
            for (var i = 0; i < categories.length; i++) {                
                ...
                category[key] = category[key] || { files: [], category: key };
                category[key].files.push(data);

                categories.pop();
            }
        }
        
        // sort category and assign metadata
        for (var key in category) {
            var settings = categoryOption[key];
            category[key]['AllCategory'] = category;
            if (!settings) {
            ...
            for (var settingKey in settings) {
                ...
                if (settingKey === 'name') continue;
                //if (settingKey === 'name') continue;
                category[key][settingKey] = settings[settingKey];
            }

            var sort = settings.sortBy || 'date';
            var col = category[key].files;
        ...
        return paginate(files, metalsmith, done, categoryOption);
    }
}
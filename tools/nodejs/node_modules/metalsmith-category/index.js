...
function createGetNextFn(files, index) {
    return function (number) {        
        var start = index + 1;
        var end = Math.min(files.length, start + number);
        return files.slice(start, end)
    }
}

function createGetPreviousFn(files, index) {
    return function (number) {        
        var start = Math.max(0, index - number);
        var end = index;
        return files.slice(start, end)
    }
}
...
module.exports = function (opts) {
    return function (files, metalsmith, done) {
        ...
        for (var filePath in files) {
            ...
            if(data.layout!= undefined && data.layout != '404.html'){
        ...
        for (var key in categoryFlatMap) {
            ...
            // create 'next' and 'previous' of current file in a category
            // file.relateInCategory[:fullCategoryName].getNext(num)
            // file.relateInCategory[:fullCategoryName] = {next: file, previous: file}
            for (var i = 0; i < col.length; i++) {
                if (!col[i].relateInCategory) col[i].relateInCategory = {};
                if (!col[i].relateInCategory[key]) col[i].relateInCategory[key] = {};
                
                col[i].relateInCategory[key].getNext = createGetNextFn(col, i);
                col[i].relateInCategory[key].getPrevious = createGetPreviousFn(col, i);

                if (i === 0) {
                    col[i].previous = null;
                    if (i > 0) col[i].relateInCategory[key].next = col[i + 1];
                    continue;
                } 
                
                if (i === col.length - 1) {
                    if (i > 0) col[i].relateInCategory[key].previous = col[i - 1];
                    col[i].next = null;
                    continue;
                }
                
                col[i].relateInCategory[key].previous = col[i - 1];
                col[i].relateInCategory[key].next = col[i + 1];
            }            
            //console.log('--------------------------');
            //console.log(col);
        }
            ...
            if (chunks.length == 1) {
                ...
                if (key != 'root') {
                    if(category.files.length > 0){
                      var files = category.files.filter(function(item){
                            return (item.layout != undefined && item.layout !="404.html");
                        });

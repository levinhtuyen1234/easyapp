...
(function (mod) {
  ...
  CodeMirror.defineMode("json-frontmatter", function (config, parserConfig) {
    var yamlMode = CodeMirror.getMode(config, 'application/json')
      ...
      token: function (stream, state) {
        if (state.state == START) {
          ...
          if (stream.match(/---\w*/, false)) {
			console.log('stream.match(/---\w*/, false');
			state.state.post =  state.state.post + 7;
			state.state.start =  state.state.start + 7;
            state.state = FRONTMATTER
            stream.skipToEnd();
            return null;
            //return yamlMode.token(stream, state.inner)
          } else {
			console.log('ELSE', stream.string);
            state.state = BODY
            state.inner = CodeMirror.startState(innerMode)
            return innerMode.token(stream, state.inner)
          }
        } else if (state.state == FRONTMATTER) {
			console.log('state.state == FRONTMATTER');			
			if (stream.string === '---json') {
				console.log('FOUND ---json');
				stream.skipToEnd();
			}
          var end = stream.sol() && stream.match(/---/, false)
          console.log('state.state == FRONTMATTER', stream);
...
CodeMirror.registerHelper("lint", "json", function(text) {
	var FORM_START = '---json';
	var FORM_END = '---';
	function getJsonContent(fileContent) {
		...
		// add \r\n thay cho ---json bi xoa
		return '\r\n' + fileContent.substr(start, end - start).trim();
	}
	
	text = getJsonContent(text);
	console.log('json lint', text);
  var found = [];
